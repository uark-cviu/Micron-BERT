...
def init_distributed_mode(args):
    ...
    # launched with submitit on a slurm cluster
        ...
        if "OFFSET" in os.environ:
            ...
            offset = 0
        args.rank = int(os.environ["SLURM_PROCID"]) + offset
...
def load_pretrained_checkpoint(model, path, trim_module_key=False):
    ...
    for k in current_state_dict.keys():
        if k in checkpoint and current_state_dict[k].shape == checkpoint[k].shape:
